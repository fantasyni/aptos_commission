...
async function modify_source(sender: Account, new_source: string): Promise<string> {
    const transaction = await aptos.transaction.build.simple({
        sender: sender.accountAddress,
        data: {
            function: `${MODULE_ADDRESS}::user::modify_source`,
...
async function main() {
    ...
    let new_source = "deepapt6";

    let r = await modify_source(user, new_source);
